...
- **`review.json` is untrusted LLM output.** The publisher sanitizes the summary markdown (strip/escape links and HTML) and **never merges** - poisoned repo content can steer verdict text, so Momos never wires `verdict` to an auto-merge.
  - **Verdict-driven reviews are opt-in (`publish.approvals`, default off, 2026-08-20).** When enabled, the publisher posts the verdict as a real GitHub review event: `APPROVE` when clean, `REQUEST_CHANGES` otherwise. This lets model output influence merge *gating* (branch protection "require approvals"), so it is off by default and fenced with guardrails: **never auto-approve a fork PR** (untrusted diff → prompt-injection risk; downgraded to `COMMENT`), **never on a stale review**, and **any `major`/`critical` finding forces `REQUEST_CHANGES`** regardless of the stated verdict (a model can't approve over its own blocking findings). Approval only ever *unblocks*; Momos still never merges. Off (the default) keeps the original advisory behavior — verdict shown in the summary only. See `internal/publisher.reviewEvent`.
