  ...
  "steps": [
    {
      "id": 1,
      "name": "Clone & Context",
      "image": "ghcr.io/ls1intum/momos-clone:1.0",
    ...
    },
    {
      "id": 2,
      "name": "AI Review",
      "image": "ghcr.io/ls1intum/momos-reviewer-oneshot:1.0",
    ...
    },
    {
      "id": 3,
      "name": "Publish",
      "image": "ghcr.io/ls1intum/momos-publisher:1.0",
...
repositories:
  - match: "ls1intum/hades"
    ...
    reviewer:
      strategy: agentic
      image: ghcr.io/ls1intum/momos-reviewer-agentic:1.0
...
- **Production:** publish to `ghcr.io/ls1intum/momos-*`; ensure the daemon has pull credentials for private images.
...
| Step | Image | Purpose |
|---|---|---|
| 1. Clone | `git-container` (reused, `ghcr.io/ls1intum/git-container`) | Full clone of the head branch into `/shared/<repo>` (`REPOSITORY_DIR=/shared` to avoid the fallback quirk). Read token via `HADES_0_USERNAME`/`HADES_0_PASSWORD`. go-git auth is transport-only, so no token persists in `.git/config`. |
...
All Momos images published to `ghcr.io/ls1intum/momos-*` and mirrored to a local registry for development (10.4).
